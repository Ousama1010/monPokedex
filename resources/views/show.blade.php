<x-guest-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-12">
        <div class="bg-gray-100 overflow-hidden shadow-md sm:rounded-lg p-8">
            ...
            <div class="flex flex-col items-center">
                <img src="{{ Storage::url($pokemon->img_path) }}" alt="Image de {{ $pokemon->name }}" class="h-80 w-80 object-cover rounded-full mb-4 border-4 border-blue-600">
                ...
                <div class="mt-8 text-left w-full">
                    ...
                    <ul class="list-none p-0 mt-2">
                        @foreach ($pokemon->primaryType->attacks as $attack)
                            <li class="flex text-gray-700 bg-white px-4 py-2 rounded-lg mb-2 items-center shadow" title="{{ $attack->description }}">
                        ...
                        @if ($pokemon->secondaryType)
                            @foreach ($pokemon->secondaryType->attacks as $attack)
                                <li class="flex text-gray-700 bg-white px-4 py-2 rounded-lg mb-2 items-center shadow" style="border-left: 4px solid {{ $pokemon->secondaryType->color }};" title="{{ $attack->description }}">
                                    <img src="{{ Storage::url($attack->img_path) }}" alt="Image de {{ $attack->name }}" class="h-12 w-12 mr-4 rounded-full border-2 border-blue-600">
                                    <div>
                                        <p class="font-semibold">{{ $attack->name }} <span class="text-xs text-gray-500">({{ $attack->type->name }})</span></p>
                                        <p class="text-sm">Dégâts: {{ $attack->damage }}</p>
